...
const DataTable = () => {
  ...
  const [allRows, setAllRows] = useState([]);  // Store all records accumulated so far
  ...
  const fetchData = async (start, end, page, pageSize) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Accumulate all records so far
        setAllRows((prevRows) => [...prevRows, ...processedData]);
  ...
  const handleFetchData = () => {
    if (startDate && endDate) {
      fetchData(startDate, endDate, page, pageSize); // Pass selected dates and pagination params
    }
  };
  ...
  const handleNextPage = () => {
    if ((page * pageSize) < totalRecords) {
      setPage(page + 1);
      fetchData(startDate, endDate, page + 1, pageSize);  // Load next set of records
    }
  };
  ...
  return (
    <Box m="20px">
      ...
      {/* CSV Export Button */}
      <CSVLink
        data={allRows}  // Use allRows (all records) for exporting
...
//               : row["O2-Flow"] || 0,
//           Cell_back_pressure:
//             row.device_data?.["Cell-back-pressure"] !== undefined &&
//             row.device_data?.["Cell-back-pressurez"] !== null
//               ? row.device_data?.["Cell-back-pressure"]
//               : row["Cell-back-pressure"] || 0,
//           H2_Pressure_outlet:
//             row.device_data?.["H2-Pressure-outlet"] ||
//             row["H2-Pressure-outlet"],
//           O2_Pressure_outlet:
//             row.device_data?.["O2-Pressure-outlet"] ||
//             row["O2-Pressure-outlet"],
//           H2_Stack_pressure_difference:
//             row.device_data?.["H2-Stack-pressure-difference"] ||
//             row["H2-Stack-pressure-difference"],
//           O2_Stack_pressure_difference:
//             row.device_data?.["O2-Stack-pressure-difference"] ||
//             row["O2-Stack-pressure-difference"],
//           Ly_Rectifier_current:
//             row.device_data?.["Ly-Rectifier-current"] ||
//             row["Ly-Rectifier-current"],
//           Ly_Rectifier_voltage:
//             row.device_data?.["Ly-Rectifier-voltage"] ||
//             row["Ly-Rectifier-voltage"],
//           Cell_Voltage_Multispan:
//             row.device_data?.["Cell-Voltage-Multispan"] !== undefined &&
//             row.device_data?.["Cell-Voltage-Multispan"] !== null
//               ? row.device_data?.["Cell-Voltage-Multispan"]
//               : row["Cell-Voltage-Multispan"] || 0,
//           RECT_CT_001:
//             row.device_data?.["RECT-CT-001"] !== undefined &&
//             row.device_data?.["RECT-CT-001"] !== null
//               ? row.device_data?.["RECT-CT-001"]
//               : row["rect_ct_001"] || 0,
//           RECT_VT_001:
//             row.device_data?.["RECT-VT-001"] !== undefined &&
//             row.device_data?.["RECT-VT-001"] !== null
//               ? row.device_data?.["RECT-VT-001"]
//               : row["rect_vt_001"] || 0,

//           PLC_TIME_STAMP:
//             row.device_data?.["PLC-TIME-STAMP"] || row["PLC-TIME-STAMP"],
//         }));
//         setRows(processedData);
//       } else {
//         throw new Error("Failed to fetch data");
...
//     {
//       field: "RECT_CT_001",
//       headerName: "RECT-CT-001",
//       width: 90,
//       valueFormatter: (params) => Number(params.value).toFixed(4),
//     },
//     {
//       field: "RECT_VT_001",
//       headerName: "RECT-VT-001",
//       width: 90,
//       valueFormatter: (params) => Number(params.value).toFixed(4),
//     },
//     {
//       field: "PLC_TIME_STAMP",
//       headerName: "PLC-TIME-STAMP",
//       width: 180,
